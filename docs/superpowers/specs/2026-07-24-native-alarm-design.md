...
알람음 행은 OS 선택기를 연다. 진동과 점점 크게는 스위치로 제공하고, 다시 알림은
선택창에서 변경한다. 설정은 일정과 함께 저장되며 일정 중복 판정에는
영향을 주지 않는다.
...
- 알림창에 다시 울릴 시간을 남기고 `출결 확인`과 `다시 알림 해제` 동작을 제공한다.
...
볼륨 위·아래 버튼은 알람 화면과 발생 건을 유지한 채 현재 알람음과 진동만 끈다.
전원 버튼은 OS 기본 동작을 유지하며 출결이나 다시 알림 상태를 변경하지 않는다.
...
- 끄기 전까지 반복 재생
- 다시 알림 간격·횟수 제한·대기 알림과 볼륨 버튼 음소거
- 재부팅과 앱 업데이트 후 예약 복구
...
## 2026-07-24 사용자 검증 후 확정 변경

### 스와이프형 알람 화면

전체 화면의 버튼 세 개를 제거한다. 화면에는 시각, 출결 동작과 원형 스와이프
손잡이를 표시하고, 손잡이를 충분한 거리만큼 밀었을 때만 알람을 종료하고 기존
Google 인증 payload 경로로 이동한다. 짧은 터치나 임계값 미만 드래그는 원위치로
복귀해 실수로 인증을 시작하지 않는다.

다시 알림 최대 횟수가 2회 이상이거나 제한 없음이고 남은 횟수가 있으면 볼륨 위·아래
버튼은 현재 전체 화면을 닫고 설정된 간격 뒤 다시 알림을 예약한다. 그 외에는 알람
화면과 발생 건을 유지한 채 소리와 진동만 끈다. 다시 알림 대기 알림의 `출결 확인`과
`다시 알림 해제` 동작은 유지한다.

### 다음 발생 한 번 건너뛰기

내 일정의 토글을 끄면 우선 해당 일정이 영구 비활성화된다. 비활성 카드 아래에는
공휴일과 과정 기간을 적용한 `M월 d일(요일) 오전/오후 h:mm부터 다시 켜기` 버튼을
표시한다. 버튼을 누르면 현재 시각 이후 가장 가까운 유효 발생 한 번의 정확한 시각을
`skippedOccurrenceAt`으로 저장하고, 일정 자체는 활성화해 그 발생만 제외한 이후
발생을 미리 예약한다.

건너뛸 발생 시각 전까지 토글은 꺼진 모습과 자동 재활성화 안내를 유지한다. 해당
시각이 지나면 별도 백그라운드 작업 없이 토글을 켜진 모습으로 계산한다. 사용자가
그 전에 토글을 직접 켜면 건너뛰기를 취소하고 가장 가까운 발생부터 다시 예약한다.
이후 발생이 없는 1회 일정에는 자동 활성화 버튼을 표시하지 않는다. 비활성 상태에서도
카드 본문은 눌러 편집할 수 있으며, 편집으로 반복 규칙이나 시각이 바뀌면 기존
건너뛰기 기준을 제거한다.

### 오늘의 알람

홈의 `오늘 예정된 동작` 제목은 `오늘의 알람`으로 바꾼다. 각 행에는
`오전 8:50 · 입실`처럼 시각과 출결 동작만 표시하고 예정·완료·시간 지남·건너뜀
배지와 건너뜀 조작을 제거한다. 기존 완료 기록과 발생 payload 검증은 인증 중복 및
오래된 알람 방어에 계속 사용하되 홈 카드에는 노출하지 않는다.
...
- 끄기: 울림만 종료
- 다시 알림: 설정 간격 뒤 일회성 발생 건 예약, 횟수 증가 및 대기 알림 표시
- 인증 시작: 울림 종료 후 MainActivity에 기존 일정 payload 전달

최대 다시 알림 횟수에 도달하면 다시 알림 버튼을 숨긴다. 볼륨 위·아래 버튼은
소리와 진동만 중지하고 전체 화면과 발생 건은 유지하며 전원 버튼은 처리하지 않는다.
...
### Task 9: 다음 발생 한 번 건너뛰기

- `AttendanceSchedule`에 nullable `skippedOccurrenceAt`을 저장하고 기존 JSON은
  `null`로 복원한다.
- `ScheduleReminderPlanner`가 비활성 일정의 다음 두 유효 발생을 계산할 수 있는
  조회 함수를 제공하고, 활성 일정에서는 `skippedOccurrenceAt`과 정확히 일치하는
  한 건만 제외한다.
- `ScheduleController`는 영구 OFF, 다음 한 건 건너뛰기, 즉시 ON을 서로 다른
  상태 전이로 저장한 뒤 직렬 재동기화한다.
- `ScheduleListScreen`은 OFF 카드도 편집 가능하게 유지하고 다음 유효 발생부터
  다시 켜는 버튼과 자동 활성화 안내를 표시한다.
- 모델 왕복, 공휴일 건너뛰기, 1회 일정, 예약 제외 및 UI 동작을 관련 Dart 테스트로
  검증한다.

### Task 10: 스와이프 알람과 조건부 볼륨 버튼

- `AlarmActivity`의 세 버튼을 드래그 임계값이 있는 원형 스와이프 손잡이로 교체한다.
- 스와이프 완료는 기존 `OPEN_ATTENDANCE` 명령만 호출한다.
- `AlarmData`의 최대·현재 다시 알림 횟수로 볼륨 버튼 동작을 판정한다. 2회 이상 또는
  제한 없음이며 남은 횟수가 있으면 `SNOOZE`, 아니면 `SILENCE`를 호출한다.
- 다시 알림 대기 알림의 출결 확인·해제 동작은 유지한다.
- Android release 빌드로 Kotlin과 리소스 결합을 검증한다.

### Task 11: 오늘의 알람 단순화

- `AttendanceScreen`의 카드 제목을 `오늘의 알람`으로 변경한다.
- 일정 행에서 상태 계산, 배지 및 건너뜀 다이얼로그를 제거하고 시각과 동작만
  표시한다.
- 완료 기록과 payload 검증 로직은 삭제하지 않는다.
- 홈 레이아웃과 일정 표시 widget 테스트를 새 문구와 단순 행에 맞춘다.

### Task 12: 재검증과 실기기 설치

- 영향받은 Dart 파일을 포맷하고 일정·홈·알람 설정 관련 테스트와
  `flutter analyze`를 실행한다.
- `flutter build apk --release --split-per-abi`로 동일 서명의 arm64 APK를 만든다.
- 연결된 Galaxy S23에 `adb install -r`로 업데이트하고 저장 일정 유지와 native
  AlarmClock 등록을 확인한다.
