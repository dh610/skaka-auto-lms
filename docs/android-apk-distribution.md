...
`build/app/outputs/flutter-apk/app-release.apk`에 생성된다. 배포 버전을 식별할 수
있도록 태그와 ABI를 포함한 이름으로 복사한다.

```sh
cp build/app/outputs/flutter-apk/app-release.apk \
  skala-attendance-v0.1.0-beta.2-arm64.apk
```

복사한 APK 안에 `arm64-v8a` 라이브러리만 포함됐는지 확인할 수 있다.

```sh
unzip -l skala-attendance-v0.1.0-beta.2-arm64.apk \
  | grep 'lib/arm64-v8a/'
shasum -a 256 skala-attendance-v0.1.0-beta.2-arm64.apk
```
...
6. 상태 조회 확인
7. 잘못된 이름·캠퍼스·반 조합이 저장되지 않는지 확인
8. 올바른 사용자 정보가 최초 설정과 정보 수정에서 저장되는지 확인
...
예를 들어 현재 `1.0.0+2007`에서 다음 빌드를 배포한다면 `+2008` 이상으로 올린다.
